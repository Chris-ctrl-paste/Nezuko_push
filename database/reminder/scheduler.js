...
const auth = require('../../botconfig.json'); //you need to make this file yourself!
...
/**
 * Creates a scheduler
 * @param {Object} client the discord.js client instance we are using to communicate with discord
 */
function Scheduler(client) {
    ...
    */
    this.setReminder = async function (userId, channel, message) {
        ...
        await channel.send(` **<@${userId}>**,I will remind you about **${reminder.message}**`);
    ...
    */
    this.snoozeReminder = async function (userId, channel, message) {
        ...
        agenda.jobs({ _id: jobId }, async (err, jobs) => {
            ...
            await channel.send(`**<@${userId}>**, on **${reminderTime.format(dateFormatString)}** I will remind you **${job.attrs.data.reminder}**`);
    ...
    this.snoozeReminders = async function (userId, channel, message) {
        ...
        agenda.jobs({ name: reminderJobName, 'data.userId': userId, nextRunAt: { $eq: null } }, async (err, jobs) => {
            ...
            if (err) {
                ...
                await channel.send(` **<@${userId}>**, I have snoozed ${jobs.length} active reminders for you`);
            }
    ...
    this.listReminders = async function (userId, channel) {
        ...
        agenda.jobs({ name: reminderJobName, 'data.userId': userId, nextRunAt: { $ne: null } }, async (err, jobs) => {
            ...
            if (err) {
                ...
                sb.appendLine(` **<@${userId}>**, I have found the following upcoming reminders for you:`)
        ...
        agenda.jobs({ _id: jobId }, async (err, jobs) => {
            ...
            job.remove(async (err) => {
                ...
                if (!err) {
                    ...
                    await channel.send(` **<@${userId}>**, I have removed your most recent reminder: **${job.attrs.data.reminder}**`);
    ...
    */
    ...
    const agenda = new Agenda({ db: { address: auth.mongourl, collection: 'agenda', options: {
